...
authenticator = IAMAuthenticator('VDpAZrhGYodgmeyEAcpDVqMJeRvz6jVwdP7lmRXQbpY2')

tts = TextToSpeechV1(authenticator=authenticator)

tts.set_service_url('https://api.us-east.text-to-speech.watson.cloud.ibm.com/instances/59999114-5c9c-4153-b45c-43c30e3af134')
...
def read_audio(ws, timeout):
    ...
    with open('myOutput.txt', 'r') as f:
            text = f.readlines()
    text = [line.replace('\n','') for line in text]
    text = ''.join(str(line) for line in text)
    with open('./myOutput.mp3', 'wb') as audio_file:
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        print(p)
        with open('myOutput.txt', 'w')as out:
            out.writelines(p)
